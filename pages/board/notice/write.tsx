...
const NoticeWrite:NextPage = () => {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const noticeResponse = await axios.post('/api/notice/noticepost', postFormData, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                },
            });
            ...
            console.log(noticeResponse.data.message);
            alert("글쓰기가 완료되었습니다.");
            router.push(`/board/notice/${noticeResponse.data.id}`);
            // 폼 초기화 및 이동 등의 후속 처리
        } catch (error) {
